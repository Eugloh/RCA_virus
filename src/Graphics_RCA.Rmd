...
packages=c("optparse","plotly","rlist","tidyverse","scales","reshape2","ggplot2","stringr","dplyr","tidyr")
...
#  [1] "tax_id"       "superkingdom" "phylum"       "class"        "order"        "family"       "genus"        "species"     
#  [9] "kingdom"      "no.rank"      "no.rank1"     "no.rank2"     "no.rank22"    "no.rank3"     "subclass"     "subfamily"   
# [17] "subkingdom"   "suborder"     "subphylum"    "subspecies"   "subtribe"     "superfamily" 
...
vect<-which(colnames(dd)%in%c("Row.names" ,"tax_id","phylum","superkingdom", "class", "kingdom", "no.rank2", "no.rank22", "no.rank3", "subclass", "order","subfamily", "suborder" , "subphylum", "subspecies", "subtribe" ,"superfamily" ))
#"Row.names" ,"tax_id", "phylum", "order","class", "kingdom", "no.rank2", "no.rank22", "no.rank3", "subclass", "subfamily", "suborder" , "subphylum", subspecies", "subtribe" ,"superfamily" 
dd<-dd[,-vect]
colnames(dd)
...
levels(restructuredData$number) <- c( "RCA.Skin.1","RCA.Skin.2", "RCA.Skin.3","RCA.Skin.4","RCA.Skin.5", "RCA.Skin.6" ,"RCA.Skin.7", "RCA.Skin.8","RCA.Skin.9","RCA.Skin.10", "RCA.Skin.11","RCA.Skin.12"  , "RCA.Vulva.1" ,"RCA.Vulva.2" ,"RCA.Vulva.3","RCA.Vulva.4","RCA.Vulva.5" , "RCA.Vulva.6" ,"RCA.Vulva.7" , "RCA.Vulva.8" , "RCA.Vulva.9",
    "RCA.Vulva.10", "RCA.Vulva.11" ,"RCA.Vulva.12"  )

restructuredData$from_ <- NA
g <- grepl("Skin", restructuredData$number)
...
Table_count$taxonomy<-paste(Table_count$taxonomy,";",sep="")
# write.csv(Table_count, paste(DataDir,"matrice_comptage_final.txt",sep="/"))

colnames(Table_count)<-c("taxonomy",levels(restructuredData$number))
...
shinyApp(
  ...
  #Widgets 
    sidebarPanel(
      ...
      selectInput("taxon_radio", width = "100%",
                  ...
                  choices=list("Base"=0,"Family"=1,"Subkingdom"=2,"genus"=3,"type"=4,"species"=5),selected = 3),
      checkboxGroupInput("hist_checkbox",
                         h4("Echantillons desires:"), width = "100%",
                         names(Table_count[,2:25]), selected = colnames(Table_count[,2:6]), inline=F),
      numericInput("sample_num", width = "100%",
                  h4("Taille du re-echantillonnage:"),
                  min=2, max=1000, value=5),
      uiOutput("slider_num"),
      width=3 ),
  ...
  #Widget change seuil max = nombre individus du re-echantillonnage
    output$slider_num <- renderUI({sliderInput("hist_num", width = "100%",
                  ...
                  min=2, max=input$sample_num, step=1, value=2)})
  #Graphique creation
    output$histogramme_c <- renderPlotly({
      ...
      # graphe_t <- graphe_trans

      graphe_t$taxonomy <- str_extract(graphe_t$taxonomy,"[^\\;]*$")
      
      #creation des nouveaux echantillons
      graphe_sample <- run_sample_table(graphe_t,input$sample_num,input$hist_checkbox)
      
      #Graphique barres empilees
      data_graphe <- data.frame(species=graphe_sample$taxonomy, melt(graphe_sample, id.vars=c("taxonomy"))) %>% select(-taxonomy)
